...
if [ -f "$SCRIPT_DIR/install-dependencies-mint.sh" ]; then
    bash "$SCRIPT_DIR/install-dependencies-mint.sh"
else
    echo -e "${RED}Error: install-dependencies-mint.sh not found${NC}"
    exit 1
fi
